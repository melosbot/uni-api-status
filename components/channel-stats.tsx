// 文件名: channel-stats.tsx
// 备注：这是一个完整的替换文件内容。请直接用以下内容覆盖 channel-stats.tsx 文件。
                            ...
                            <TableRow key={index}>
                              ...
                              <TableCell className="text-right font-mono text-sm">
                                 <Tooltip>
                                     ...
                                     <TooltipContent>
                                         <p>成功: {channel.successes.toLocaleString()}</p>
                                         <p>失败: {channel.failures.toLocaleString()}</p>
                                     </TooltipContent>
                                 </Tooltip>
                              </TableCell>
                    ...
                    {data.map((channel, index) => (
                        <Card key={index} className="max-w-full">
                          <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2 pt-4 px-4">
                             {/* Title takes full width initially */}
                             <CardTitle className="text-sm font-medium truncate pr-2" title={channel.provider}>
                               {channel.provider}
                             </CardTitle>
                             {/* Success Rate Badge - positioned absolute or flex end if needed */}
                             {/* Or keep it within the content */}
                          </CardHeader>
                                        ...
                                        <Tooltip>
                                          <TooltipTrigger asChild>
                                              ...
                                              <span className="font-mono">
                                                  {formatNumberShort(channel.promptTokens)}/{formatNumberShort(channel.completionTokens)}/{formatNumberShort(channel.totalTokens)}
                                              </span>
                                          </TooltipTrigger>
